...
function mp_stacks_woogrid_price_meta_options( $items_array ){		
	...
	$new_fields = array(
		...
		'woogrid_price_google_font' => array(
			'field_id'			=> 'woogrid_price_google_font',
		...
		'woogrid_price_google_font_weight_style' => array(
			'field_id'			=> 'woogrid_price_google_font_weight_style',
...
/**
 * Add the Google Fonts for the Grid Prices
 *
 ...
 */
function mp_stacks_woogrid_price_google_font( $css_output, $post_id, $first_content_type, $second_content_type ){
	...
	$woogrid_price_googlefont = mp_core_get_post_meta( $post_id, 'woogrid_price_google_font' );
	$woogrid_price_googlefontweight = mp_core_get_post_meta( $post_id, 'woogrid_price_google_font_weight_style' );
	
	//If a font name has been entered
	if ( !empty( $woogrid_price_googlefont ) ){
		
		//Check if a font extra (weight) has been selected and add it if so.
		$woogrid_price_googlefontweight = isset($woogrid_price_googlefontweight) && !empty( $woogrid_price_googlefontweight ) ? ':' . $woogrid_price_googlefontweight : NULL;
		$woogrid_price_googlefont = $woogrid_price_googlefont . $woogrid_price_googlefontweight;
	
		//Load the Google Font using the Google Font Class in MP Core
		new MP_CORE_Font( $woogrid_price_googlefont, $woogrid_price_googlefont, $mp_core_google_font_args );
		$mp_stacks_footer_inline_css[$woogrid_price_googlefont] = $mp_core_font_families[$woogrid_price_googlefont];
		
		//Return the incoming css string plus css to apply this font family to all paragraph tags
		$css_output .=  '#mp-brick-' . $post_id . ' .mp-stacks-woogrid-item-price, #mp-brick-' . $post_id . ' .mp-stacks-woogrid-item-price * { font-family: \'' . $woogrid_price_googlefont . '\';}';
	
	}
	
	return $css_output;	
}
add_filter('mp_brick_additional_css', 'mp_stacks_woogrid_price_google_font', 10, 4);	